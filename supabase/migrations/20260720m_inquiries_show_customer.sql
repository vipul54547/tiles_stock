-- 20260720m — the order list must say WHO the order is for.
--
-- 👥 `inquiries.customer_id` was being saved correctly, but `my_inquiries` never returned that
-- customer's NAME — it only ever emitted `customer_hint`, the free note. So an order booked for
-- "pratap ceramic, gorakhpur" showed in Inquiries as just "new order". The customer was in the
-- database and invisible in the app.
--
-- 📕 It also now says how much of an order is still TO BE MADE (`book_boxes`), so a production
-- order can be told apart from a stock order at a glance instead of looking identical.
...
as $function$
  ...
  from (
    select jsonb_build_object(
      ...
      -- 👥 the SAVED customer this order belongs to (null when it is a walk-in note only)
