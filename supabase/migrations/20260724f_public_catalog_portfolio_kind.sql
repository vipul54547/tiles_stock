-- 20260724f — 🖼️ MEDIA PORTFOLIO, P1 slice 3l: public_catalog is kind-aware.
--
-- A PORTFOLIO catalogue (stock_catalogs.kind='portfolio', one catalogue_brand_id) opened via its own
-- list token now (catalog-token path only):
--   1. SCOPES designs to that brand — only holdings of catalogue_brand_id (#11/#15).
--   2. ALIASES each design's name to that brand's cover word.
--   3. Emits `catalog.kind` so the /s/ frontend renders it STOCK-BLIND (hides price/stock).
-- Everything else (stock lists, the stockist-token path) is unchanged.
--
-- ⚠️ MVP: still sources from holdings (in-stock designs). Showing zero-stock PRODUCTS of the brand
-- (true stock-blind range) is a documented follow-up — it needs a stockist_library-sourced query.
