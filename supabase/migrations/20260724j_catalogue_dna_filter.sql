-- 20260724j — 🖼️ MEDIA PORTFOLIO: catalogue DNA facet (re-applies public_catalog).
--
-- Adds the FULL DNA facet set to a catalogue (#13) — Look Type · Design Joint · Print Type · Colour ·
-- Punch · Application · Use Type · Behaviour · Series. filter_dna holds selected dna_value ids;
-- matching mirrors the buyer facet rule: WITHIN an attribute OR, ACROSS attributes AND (a design
-- passes only if, for every attribute that has a selected value, it carries at least one of that
-- attribute's selected values — read via _dna_of_library = print ∪ piece). Empty = no DNA filter.
-- (Builds on 20260724i/f; the rest of the function is identical.)
         ...
         where d.stockist_id = c.stockist_id
           ...
           and case
             when coalesce(c.list_type,'permanent') = 'permanent' then
               ...
               and (array_length(c.filter_dna,1) is null or not exists (
                 select 1
                   from (select distinct dv.attribute_id from dna_values dv where dv.id = any(c.filter_dna)) attrs
                  where not exists (
                    select 1 from _dna_of_library(d.library_id) ld
                    join dna_values dv2 on dv2.id = ld.value_id
                   where dv2.attribute_id = attrs.attribute_id and dv2.id = any(c.filter_dna))))
