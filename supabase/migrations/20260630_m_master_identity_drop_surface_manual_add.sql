-- Manual "Add design" (My Design Library): M duplicate guard no longer keys on
-- surface — a tile is one master per name+size. Surface is an attribute, not
-- identity. See project_per_brand_stock.
...
begin
  ...
  -- Duplicate guard. M: a tile is ONE box per name+size (brand-agnostic; surface is
  -- an attribute, not identity) — creating the same tile under a 2nd brand is
  -- blocked; add the brand name by EDITING the existing design.
  -- T/W: brand silo — unique within the brand by name+size.
  if v_type = 'M' then
    if exists (select 1 from stockist_library
      ...
      raise exception 'This tile "%" (%) is already in your library — open it to add another brand''s name', v_name, v_size;
    end if;
  else
    if exists (select 1 from stockist_library
               where stockist_id = v_stk and lower(master_design_name) = lower(v_name)
  ...
  -- Identity attributes (overwrite where provided; NULL param = leave as-is).
