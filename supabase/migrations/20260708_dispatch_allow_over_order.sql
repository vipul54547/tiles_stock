-- Reverses the over-dispatch cap from 20260708_dispatch_block_over_order.
-- User decision 2026-07-08: dispatch is the FINAL PHYSICAL TRUTH and must not be
-- blocked. A stockist legitimately ships MORE than the buyer ordered when the
-- buyer bumped their quantity, or when the stockist clears the last 3-4 boxes
-- rather than leave them in the godown. Blocking it broke real dispatches.
-- This does NOT reintroduce negatives: remaining is still greatest(quantity −
-- dispatched_qty, 0) → an 85-order shipped 100 shows Ordered 85 / Dispatched 100
-- / Remaining 0. The extra is honestly recorded, nothing goes negative.
-- Restores the pre-cap body (== 20260707_order_flow_simplify_gates_hold_dispatch:
-- relaxed gate + basket materialize + v_total messaging), cap removed.
