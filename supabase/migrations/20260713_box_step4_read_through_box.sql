-- BOX chapter — STEP 4 of 5: everyone READS the spec through the box.
-- (docs/BOX_AND_DERIVED_THICKNESS_PLAN.md)
--
-- 11 places read pieces_per_box / box_weight_kg off the PRODUCT. But every one of them is
-- looking at a HOLDING, and a holding carries a brand_id — so it already knows exactly which
-- box it is. Resolve through it, and a buyer looking at Brand A's stock sees Brand A's packing.
--
-- ORDERING / SAFETY. The writers (library_upsert_master, admin_library_upsert,
-- _library_apply_identity, import_stock_batch) still write the spec to the PRODUCT. If reads
-- moved to the box while writes stayed on the product, every new spec would vanish. So this
-- migration also installs a BRIDGE TRIGGER: a spec written to the product is synced down to
-- all of its boxes. Nothing breaks at any point.
--
-- Step 5 rewrites the writers to target the box directly and drops both the bridge and the
-- product columns.

-- ── the box for a holding ────────────────────────────────────────────────────────────────
-- Falls back to ANY box of the product when the holding has no brand (or its box is missing),
-- so a spec is never lost just because a brand link is absent.
...
as $function$
  select coalesce(
    (select a.pieces_per_box from stockist_library_brand_names a
      where a.library_id = p_library_id and a.brand_id = p_brand_id
        and coalesce(a.pieces_per_box,0) > 0),
    (select a.pieces_per_box from stockist_library_brand_names a
      where a.library_id = p_library_id and coalesce(a.pieces_per_box,0) > 0
...
as $function$
  select coalesce(
    (select a.box_weight_kg from stockist_library_brand_names a
      where a.library_id = p_library_id and a.brand_id = p_brand_id
        and coalesce(a.box_weight_kg,0) > 0),
    (select a.box_weight_kg from stockist_library_brand_names a
      where a.library_id = p_library_id and coalesce(a.box_weight_kg,0) > 0
      order by a.created_at limit 1)
  );
$function$;

-- ── the BRIDGE (temporary — dropped in step 5) ───────────────────────────────────────────
-- Writers still set the spec on the product. Push it down to every box of that product so the
-- new box-based reads see it. This preserves today's exact behaviour: one spec per product,
-- shared by its brands. Differentiating them per brand is what the box chip UI will do.
create or replace function public._trg_sync_product_spec_to_boxes()
 returns trigger
 language plpgsql
 security definer
 set search_path to 'public', 'pg_temp'
as $function$
begin
  if coalesce(new.pieces_per_box,0) > 0 or coalesce(new.box_weight_kg,0) > 0 then
    update stockist_library_brand_names a
       set pieces_per_box = coalesce(nullif(new.pieces_per_box,0), a.pieces_per_box),
           box_weight_kg  = coalesce(nullif(new.box_weight_kg,0),  a.box_weight_kg)
     where a.library_id = new.id
       and (a.pieces_per_box is distinct from coalesce(nullif(new.pieces_per_box,0), a.pieces_per_box)
         or a.box_weight_kg  is distinct from coalesce(nullif(new.box_weight_kg,0),  a.box_weight_kg));
  end if;
  return new;
end; $function$;

drop trigger if exists zz_sync_product_spec_to_boxes on stockist_library;
create trigger zz_sync_product_spec_to_boxes
  after insert or update of pieces_per_box, box_weight_kg
  on stockist_library
  for each row execute function _trg_sync_product_spec_to_boxes();



-- ── view public_designs: pieces/weight resolve through THE BOX (2 refs) ──
create or replace view public.public_designs as
 SELECT d.id,
  ...
  WHERE s.is_active = true AND d.status <> 'out_of_stock'::text AND d.box_quantity > 0 AND (EXISTS ( SELECT 1
           FROM stock_catalogs c
          WHERE c.stockist_id = d.stockist_id AND c.visibility = 'public'::text AND c.show_in_marketplace = true AND c.is_active = true AND (COALESCE(c.list_type, 'permanent'::text) = 'temporary'::text AND (EXISTS ( SELECT 1
...
-- ── view market_designs: pieces/weight resolve through THE BOX (2 refs) ──
create or replace view public.market_designs as
 SELECT d.id,
  ...
  WHERE s.is_active = true AND s.is_listed = true AND d.status <> 'out_of_stock'::text AND (d.box_quantity - d.control_quantity - held_of(d.id)) > 0 AND (EXISTS ( SELECT 1
           FROM stock_catalogs c
          WHERE c.stockist_id = d.stockist_id AND c.visibility = 'public'::text AND c.show_in_marketplace = true AND c.is_active = true AND (COALESCE(c.list_type, 'permanent'::text) = 'temporary'::text AND (EXISTS ( SELECT 1
                   FROM catalog_designs cd
                  WHERE cd.catalog_id = c.id AND cd.library_id = d.library_id)) OR COALESCE(c.list_type, 'permanent'::text) = 'permanent'::text AND (array_length(c.filter_brand_ids, 1) IS NULL OR (COALESCE(d.brand_id, lib.brand_id) = ANY (c.filter_brand_ids))) AND (array_length(c.filter_qualities, 1) IS NULL OR (d.quality = ANY (c.filter_qualities))) AND (array_length(c.filter_surfaces, 1) IS NULL OR (d.surface_type = ANY (c.filter_surfaces))) AND (array_length(c.filter_sizes, 1) IS NULL OR (d.size = ANY (c.filter_sizes))) AND (array_length(c.filter_tile_types, 1) IS NULL OR (lib.tile_type = ANY (c.filter_tile_types))) AND (array_length(c.filter_stock_types, 1) IS NULL OR (effective_stock_type(lib.stock_type, d.quality) = ANY (c.filter_stock_types))) AND (c.filter_box_min IS NULL OR GREATEST(0, d.box_quantity - d.control_quantity - held_of(d.id)) >= c.filter_box_min) AND (c.filter_box_max IS NULL OR GREATEST(0, d.box_quantity - d.control_quantity - held_of(d.id)) <= c.filter_box_max))));

-- ── my_stock: pieces/weight through THE BOX (2 refs) ──
...
-- ── my_private_designs: pieces/weight through THE BOX (2 refs) ──
...
-- ── public_catalog: pieces/weight through THE BOX (4 refs) ──
