-- BUGFIX: photo-carrying imports duplicated every M master.
--
-- import_stock_batch attached a row's photo via library_contribute(p_brand_id,…),
-- which re-looks-up the master by brand_id. But library_map_upsert (run one line
-- earlier) already created/returned the master with brand_id = NULL for an M
-- stockist. So the brand-scoped lookup MISSED that master and INSERTED a SECOND,
-- brand-owned master (+ a duplicate alias) in the same transaction — one row that
-- held the stock but no image, plus a twin that held the image. (T/W was safe:
-- its masters ARE brand-scoped, so the lookup matched.)
--
-- Fix: attach the photo DIRECTLY to v_master — the master library_map_upsert
-- already returned, authoritative for both M and T/W — with first-writer-wins.
-- Drops the fragile brand re-lookup entirely; library_contribute is no longer used
-- by the import path. (project_per_brand_stock)
    ...
    with z as (
      update designs set box_quantity = 0, updated_at = now()
       where stockist_id = v_stk and box_quantity <> 0 and not (id = any(v_touched))
         and (p_brand_id is null or brand_id is not distinct from p_brand_id)
