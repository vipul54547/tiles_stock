-- Per-row multi-brand "Fully new": a single Excel file can carry rows for
-- several brands at once (Option 2/3), so "this brand only" is meaningless —
-- the file isn't a single-brand file. Add p_wipe_brand_ids so the caller can
-- say "wipe exactly the brands present in this file". Precedence:
--   p_wipe_all_brands  -> every brand
--   p_wipe_brand_ids   -> just those brands (per-row multi-brand case)
--   else p_brand_id    -> single-brand upload (unchanged behaviour)
...
begin
  ...
  -- Zero out every OTHER untouched design in scope. Precedence: all brands,
  -- else an explicit brand-id list (per-row multi-brand file), else the single
  -- fallback brand.
...
-- Drop the previous 7-arg overload so only the 8-arg version resolves.
drop function if exists public.import_stock_batch(uuid, uuid, uuid, text, jsonb, text, boolean);
