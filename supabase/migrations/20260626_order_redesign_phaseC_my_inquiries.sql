-- Phase C — surface connection_code/customer_hint/source in the stockist order
-- list, and make it null-buyer-safe (web/stockist orders have no end_user).
...
as $function$
  ...
  from (
    select jsonb_build_object(
      ...
      -- App basket (my_choices) only applies to an app order still open; every
      -- other case (locked+, or any null-buyer web/stockist order) reads items.
      'line_count', case when i.end_user_id is not null and i.status in ('draft','sent','confirmed')
      ...
      'total_boxes', case when i.end_user_id is not null and i.status in ('draft','sent','confirmed')
      ...
      'designs', case when i.end_user_id is not null and i.status in ('draft','sent','confirmed')
      ...
      'brands', case when i.end_user_id is not null and i.status in ('draft','sent','confirmed')
