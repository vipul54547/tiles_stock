-- 20260720e — 🚫 ADDING STOCK MAY NEVER CREATE A BOX.
--
-- `_box_for` INSERTED a missing box. So the stock counter could invent a cover: pick the wrong
-- brand and the server silently declared "ANUJ wraps this tile" — a business fact nobody decided,
-- which then showed up in the Library and on the public page as if it were real.
--
-- 🔑 This is the same law that already governs the product: **a stock path must never create
-- identity or the structure around it.** It is how the 444 bad rows happened (a stock import was a
-- product factory). Adding stock must not mint a DESIGN — and now, not a BOX either.
--
-- 🎁 A BOX is DECLARED IN THE LIBRARY, by `box_put_cover` — that is the only writer left. He ticks
-- the brand on the design, with the box in front of him. Never mid-import, never at the counter.
--
-- Two callers, and they need different answers, so they get the pair this codebase already uses
-- for products (`library_map_upsert` vs its read-only twin `library_map_resolve`):
--   * `stock_add_holding` — a HUMAN is standing there, so RAISE in plain English (`_box_for`).
--   * `import_stock_batch` — no human, and one bad row must not throw the whole batch, so RESOLVE
--     to NULL (`_box_resolve`) and REPORT the row in `unmatched_rows`, exactly as an unresolved
--     product already is.

-- ── the read-only twin: which box, or NULL. Creates nothing. ─────────────────────────────────
create or replace function public._box_resolve(
...
begin
  if p_brand_id is null then return null; end if;
  ...
  if v_packing is null then return null; end if;

  select id into v_box from boxes where packing_id = v_packing and brand_id = p_brand_id;
  return v_box;
end $function$;

-- ── the counter's version: resolve, or say why not. NO INSERT. ───────────────────────────────
create or replace function public._box_for(
 ...
 set search_path to 'public', 'pg_temp'
as $function$
declare v_packing uuid; v_box uuid; v_brand text; v_design text;
begin
  ...
  -- 🚫 No cover, no box, and stock does NOT get to make one.
  if v_box is null then
    select name into v_brand from brands where id = p_brand_id;
    ...
    raise exception
      ...
      coalesce(v_brand,'That brand'), coalesce(v_design,'this design'),
...
-- ── the importer: an uncovered row is REPORTED, never minted. ────────────────────────────────
...
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    ...
    if not coalesce(p_library_only, false) and v_qty > 0 and v_master is not null then
      ...
      --
      -- 🚫 And an import may NOT create that box. No cover means the brand does not wrap this
      -- design — that is a fact for the Library, not something a spreadsheet gets to declare. The
      -- row is REPORTED, exactly like a row whose design could not be resolved.
      v_box := _box_resolve(v_master, v_hold_brand, null);
      if v_box is null then
        v_unmatched := v_unmatched + 1;
        v_unmatched_rows := v_unmatched_rows || jsonb_build_object(
          'name', v_name, 'size', v_size, 'surface', v_surface, 'quality', v_quality,
          'reason', format('%s has no cover for this design — tick it on the design in your '
                           'Design Library first.',
                           coalesce((select name from brands where id = v_hold_brand),
                                    'That brand')));
        continue;
      end if;
...
-- Self-check: nothing but box_put_cover may insert a box.
do $$
declare n text;
begin
  select string_agg(p.proname, ', ') into n
  from pg_proc p join pg_namespace ns on ns.oid = p.pronamespace
  where ns.nspname = 'public' and p.prosrc ilike '%insert into boxes%'
    and p.proname <> 'box_put_cover';
  if n is not null then
    raise exception 'These still create a box: %', n;
  end if;
end $$;
