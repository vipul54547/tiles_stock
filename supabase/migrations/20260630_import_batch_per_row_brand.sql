-- import_stock_batch: the holding's brand can be set PER ROW (row.brand_id), so an
-- M Option-2 sheet with one brand column filled per row stocks each row under THAT
-- brand instead of the global upload brand. Priority: row.brand_id > row alias
-- brand > batch brand > master's own brand. (project_per_brand_stock)
