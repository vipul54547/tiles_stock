-- import_stock_batch writes the brand onto each holding so Excel/PDF imports build
-- per-brand stock. Holding brand = the row's alias brand (per-row M), else the
-- import's brand, else the master's own brand (T/W). (project_per_brand_stock)
...
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    ...
    if v_qty > 0 and v_master is not null then
      -- Brand these boxes are stocked under: the row's alias brand (per-row M),
      -- else the import's brand, else the master's own brand (T/W).
      v_hold_brand := coalesce(nullif(v_aliases->0->>'brand_id','')::uuid, p_brand_id,
