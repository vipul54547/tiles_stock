-- M identity redesign — POLISH: pass the row surface into the surface-aware
-- library_map_upsert (see 20260624_library_map_upsert_surface_aware.sql).
-- Only line that changes vs 20260622_import_stock_batch_dna_merge.sql is the
-- library_map_upsert call (now 4-arg with v_surface). Reversible: re-apply the
-- 20260622 migration after restoring the 3-arg library_map_upsert.
...
begin
  ...
  for r in select * from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    ...
    -- SURFACE-AWARE: the box identity for M is master + surface.
