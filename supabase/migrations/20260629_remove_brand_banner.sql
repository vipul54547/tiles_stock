-- Remove the brand-level banner entirely. Banners now live only on the stock
-- list (per-list banner_source/bg/logo/pos) with the legacy single-image and the
-- generic pool as fallbacks. The brand keeps only its identity (name + logo).
-- (project_dispatch_order_redesign sibling cleanup; brand banner retired)

-- 1. public_catalog: drop the brand-banner fallback branch. Banner precedence is
--    now: per-list rich banner → legacy per-list single image → generic pool.
...
-- 2. Drop the brand-banner RPCs (admin-only editors, now retired).
drop function if exists public.admin_set_brand_banner_config(uuid, text, text, text, text, text);
drop function if exists public.admin_set_brand_banner(uuid, text);
drop function if exists public.admin_clear_brand_banner(uuid);
drop function if exists public.admin_set_brand_use_pool(uuid, boolean);
drop function if exists public.admin_stockist_banner_slots(text);

-- 3. Drop the brand-banner columns. Brand keeps name + logo_url only.
alter table public.brands
  drop column if exists banner_source,
  drop column if exists banner_bg_url,
  drop column if exists company_logo_url,
  drop column if exists company_pos,
  drop column if exists td_pos,
  drop column if exists use_pool_banner;
