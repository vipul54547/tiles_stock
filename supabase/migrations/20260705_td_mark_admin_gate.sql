-- TilesDesign mark: admin gates SHOW/HIDE (per stockist), stockist chooses WHERE
-- (per stock-list, default top-right, no "None"). Reverses the old model where the
-- stockist could hide the platform mark entirely.
--
--   • stockists.td_show (bool, default FALSE) — admin toggle. Mark renders only
--     when this is on. Applies to ALL banner sources (pool/library/upload).
--   • stock_catalogs.td_pos — stockist's position, default 'top-right'; legacy
--     'none'/'' coerces to 'top-right' (a stockist can move it, never hide it).
--   • public_catalog now returns td_show + a coerced td_pos on every banner.

-- 1. Admin show/hide flag (default OFF).
alter table public.stockists
  add column if not exists td_show boolean not null default false;

-- 2. Position: default to top-right for new lists; heal empty/none existing rows.
alter table public.stock_catalogs alter column td_pos set default 'top-right';
update public.stock_catalogs
   set td_pos = 'top-right'
 where td_pos is null or btrim(td_pos) = '' or td_pos = 'none';

-- 3. Admin RPC to flip the mark on/off for a stockist.
create or replace function public.admin_set_stockist_td(p_seq text, p_show boolean)
 returns void
 language plpgsql
 security definer
...
begin
  ...
  update public.stockists set td_show = coalesce(p_show, false)
   where sequential_id = p_seq;
end;
$function$;

revoke execute on function public.admin_set_stockist_td(text, boolean) from public;
grant  execute on function public.admin_set_stockist_td(text, boolean) to authenticated;

-- 4. public_catalog: add td_show + coerced td_pos ('none' -> 'top-right') to every
--    banner branch. (Rest of the function is unchanged from the live definition.)
  ...
  select coalesce(
    (select jsonb_build_object(
       ...
       'banner', case
         ...
         then jsonb_build_object('source','custom','bg_url',c.banner_url,'image_url',c.banner_url,'overlay',false,'company_logo_url',null,'company_pos','none','td_pos', case when coalesce(nullif(btrim(c.td_pos),''),'none') = 'none' then 'top-right' else c.td_pos end,'td_show', s.td_show,'name',c.name)
         else jsonb_build_object('source','pool','bg_url',pick_generic_banner(s.id::text),'image_url',pick_generic_banner(s.id::text),'overlay',true,'company_logo_url',null,'company_pos','none','td_pos', case when coalesce(nullif(btrim(c.td_pos),''),'none') = 'none' then 'top-right' else c.td_pos end,'td_show', s.td_show,'name', s.name) end,
