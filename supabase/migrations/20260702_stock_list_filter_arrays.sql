-- Upgrade permanent list filter columns from single-value text to text arrays.
-- Adds: tile_types, stock_types filter arrays + box range (min/max F-stock).
-- Empty array = no filter (replaces the old single-null sentinel).

-- 1. Drop old single-text filter columns (all NULL: every existing list is 'temporary').
alter table public.stock_catalogs
  drop column if exists filter_quality,
  drop column if exists filter_surface,
  drop column if exists filter_size;

-- 2. Add array filter columns.
alter table public.stock_catalogs
  add column if not exists filter_qualities   text[] not null default '{}',
  add column if not exists filter_surfaces    text[] not null default '{}',
  add column if not exists filter_sizes       text[] not null default '{}',
  add column if not exists filter_tile_types  text[] not null default '{}',
  add column if not exists filter_stock_types text[] not null default '{}',
  add column if not exists filter_box_min     int,
  add column if not exists filter_box_max     int;

-- 3. Updated stock_list_save: array params, empty array = no filter.
  ...
  if p_id is null then
    ...
    insert into stock_catalogs
      (stockist_id, brand_id, name, description, visibility, show_in_marketplace,
       sort_order, is_anonymous, list_type, filter_brand_id,
...
-- 4. Updated public_catalog: array matching (array_length IS NULL = empty = no filter).
