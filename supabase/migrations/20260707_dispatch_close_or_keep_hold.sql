-- Phase 1 of the "order remaining" model: the stockist decides at dispatch
-- whether to CLOSE the order (release the remaining hold → buyer re-orders the
-- rest) or KEEP it open (remaining stays reserved/held → buyer just waits).
--   p_close = true  → close: order 'completed', remaining hold released.
--   p_close = false → keep open: order 'dispatching' (Part-N), remaining held.
-- Collapse the two legacy overloads into one canonical signature; the two
-- trailing booleans default so older app builds (named-arg calls) still resolve.
-- Applied to project buxjebeeiwyrsakeucyk 2026-07-07 (round-trip verified:
-- close → completed/held_qty 0/held_of 0; keep → dispatching/held_of 60).
drop function if exists public.dispatch_inquiry(uuid,jsonb,text,text,text,text,date);
drop function if exists public.dispatch_inquiry(uuid,jsonb,text,text,text,text,date,boolean);
...
begin
  ...
  -- An order closes when fully dispatched, OR when the stockist closes the
  -- remaining (p_close). It stays open (Part-N) only when the stockist keeps the
  -- remaining held for this buyer.
  if v_dispatched > 0 and (v_outstanding = 0 or p_close) then
    v_new_status := 'completed';
    -- Release any remaining hold (a completed order is already excluded from
    -- held_of(), but zero it so nothing lingers as reserved).
